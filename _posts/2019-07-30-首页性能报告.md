...
## 工具
#### ["严格模式"](https://www.jianshu.com/p/271474cd1d91)
...
1. 通过Android的“严格模式”在日志中排查主线程中是否有io操作，其中发现三个异常问题，这也是QA很难发现但确实存在的问题。
...
2. 通过“严格模式”也可以看出模块布局资源和图片加载耗时的具体数据，加载布局和图片的过程其实都有在主线程中进行io操作。
