...
~~~
可以看见，ViewPager2和ViewPager在预加载的实现上截然不同：ViewPager默认预加载值就是1，如果外部设置小于1的话会直接抛出异常；ViewPager2提供了一个负数作为默认值（早期版本这个值是0，当前的版本是-1），然后将该值提供给RecyclerView处理自定义的预加载。ViewPager2如果没有这个默认值小于1（对比ViewPager的默认值）的设计，其实它跟ViewPager的预加载没有本质的差别，但是一旦有了这个后门，就提供了可操作的空间。
