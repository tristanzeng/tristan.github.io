...
这个问题其实不是一个单选，具体在应用场景中，最终两者都有用到。下滑到达顶部、上滑到达底部，此时需要触发下拉刷新和加载更多操作，随即下拉刷新和加载更多模块被带动，那么就实现了`思路1`的做法；而其他位置的上滑和下滑，都不会触发这两个操作，所以可以理解为将其暂时卸载，那么就有了`思路2`的做法。整体首页的实现，其实是综合应用了这三种思路。
