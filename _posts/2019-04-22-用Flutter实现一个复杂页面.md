...
```
首页主体控件使用了NestedScrollView的扩展类ExtendedNestedScrollView，前者允许嵌套滚动，但是对子视图的高度有要求——确定的高度。我们知道，feed流的高度是很难计算的，因为它本身就是会一直扩展的，高度会一直变化。所以，基于NestedScrollView的扩展类ExtendedNestedScrollView解决了这个痛点。

再来关注一下它的这个属性keepOnlyOneInnerNestedScrollPositionActive，直译是仅保活一个内部嵌套的滚动位置，意译便是仅允许一个内部嵌套的视图滚动，即仅允许一个轮子驱动。
