...
变量collapsedHeight代表了折叠后头部的高度，从它的计算表达式里看出：当widget.bottom == null的时候，collapsedHeight的值为null。换句话说，如果不使用bottom，那么折叠高度是没办法自定义的。最后，就是使用bottom做头部容器了。
...
进一步验证，如果collapsedHeight == null，那么折叠后的头部高度就是topPadding + kToolbarHeight的值了。topPadding是系统状态栏的高度，kToolbarHeight是个常量，这也解释了为什么不使用bottom折叠头部的高度是一个固定值的原因了。
