...
## 技术难点
#### 两级吸顶
在Flutter中实现吸顶功能比较容易，使用SliverPersistentHeader控件或者间接使用该控件就能满足需要；更重要的是，它可以满足滑动过程中任意组件的吸顶功能，通俗地说，也就是多级吸顶。

既然多级吸顶都可以完成，那么两级吸顶就很轻松了。第一级，可以使用SliverAppBar（如果查看源码的话，它其实就是一个SliverPersistentHeader控件），不仅可以吸顶，还带有折叠属性，折叠属性能更好的满足头部滑动时的动效处理；第二级，使用SliverPersistentHeader并自定义它的delegate，通过pinned属性灵活选择当前模块吸顶与否，这样就完美实现了两级头部和feed流tab的吸顶效果。
