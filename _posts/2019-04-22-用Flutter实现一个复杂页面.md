...
## 技术难点
#### 两级吸顶
在Flutter中两级吸顶是比较容易实现的：第一级，可以用AppBar，不仅可以吸顶，还带有折叠属性，折叠属性能更好的满足头部滑动时的动效处理；第二级，可以使用SliverPersistentHeader，通过pinned属性灵活选择当前模块吸顶或是不吸顶，这样就完美实现了二级feed流tab的吸顶效果。<br/>
而native端实现这个二级吸顶却很费力，通常你可能需要在滑动到顶部前隐藏一个跟吸顶内容一样的view在那里，需要自己计算是否到顶部并维护view的可见性。
```js
SliverAppBar(
  pinned: true,
  ...,
