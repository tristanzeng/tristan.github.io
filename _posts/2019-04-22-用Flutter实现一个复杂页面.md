...
const double CITY_BOTTOM         = 60.0; // 城市名bottom下边距
...
class HeaderState extends State<Header> with TickerProviderStateMixin {
  ...
  drive(offset) {
    ...
    ...
    // 城市名距离底部位置
    widget.cityBottom = CITY_BOTTOM * (1 - percent) + 6 * (percent);
    // 背景颜色控制
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: <Widget>[
        Positioned(...),// 底部白条
        SearchBar(bgColor: searchBgColor.value, ...),// 搜索栏
        Positioned(bottom: widget.cityBottom, ...),// 城市
        Positioned(bottom: widget.cityBottom, ...),// 扫码
        Positioned(bottom: widget.cityBottom, ...),// 签到
      ],
    );
  }
