...
加载更多的原理其实跟native的思路是一样的——判断列表滚动到最末位置触发特定事件。之前native的做法就是判断RecyclerView滑动到最后一项时向feed流最末位置插入一个特定的动画模板，等加载结束后再把这个模板去掉，然后把请求到的内容添加到视图列表中，这样就完成了一个完整的加载更多功能。
...
## 总结
这样，一个由Flutter开发的首页就已经基本实现了。

## 参考文档
(Flutter 扩展NestedScrollView)[https://juejin.im/post/5beb91275188251d9e0c1d73]
