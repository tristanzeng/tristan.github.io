...
源码看，如果collapsedHeight == null，那么折叠后的头部高度就是topPadding + kToolbarHeight了。topPadding是系统状态栏的高度，kToolbarHeight是个自定义常量。不难看出，bottom为空时折叠头部的高度就会是一个固定高度。那么反过来，想要自定义高度，就必定使用bottom做头部容器了，折叠后的头部高度则完全取决于bottom的高度（一般，系统状态栏的高度是确定的）。
