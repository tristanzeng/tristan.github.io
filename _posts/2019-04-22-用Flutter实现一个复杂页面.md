...
```
以上是改动的代码逻辑，主要是通过位置动画限定了首页主体向下滚动的最大位移。同时，通过动画偏移的计算，向外输出了头部偏移值，以便于外部通过监听这个偏移值，做更多的动效处理，比如：搜索框、天气、城市、头部、扫码、背景图等头部元素的动效。

负二楼的效果实现其实并不复杂，能理解如何通过动画原理改动下拉刷新控件从而实现个性化的动效，那么实现负二楼的效果其实就是举一反三的事情。
