...
#### AppBar折叠原理
AppBar构造方法中有四个Widget类型的参数。
> * leading
> * title
> * flexibleSpace
> * bottom

看到折叠后高度collapsedHeight的计算表达式，你可能会明白为什么在上面SliverAppBar中会特意留了bottom属性（ps.之前一直在研究搜索栏是不是应该使用title属性，因为这两个都是Widget类型），当widget.bottom != null的时候，collapsedHeight会包括bottom的高度，这决定了头部需要使用bottom属性。
```js
