...
## 技术难点
#### 两级吸顶
在Flutter中两级吸顶是比较容易实现的。第一级，可以用AppBar，不仅可以吸顶，还带有折叠属性，折叠属性能更好的满足头部的一些动效处理；第二级，可以使用SliverPersistentHeader，通过pinned属性可以灵活选择当前模块吸顶或是不吸顶，这样就完美实现了feed流tab的吸顶效果。而native端需要实现这个二级吸顶还是很费力的。
```js
SliverAppBar(
  pinned: true,
  ...,
