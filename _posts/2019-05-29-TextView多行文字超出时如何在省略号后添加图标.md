...
#### Layout属性
TextView的Layout属性管理了文本的布局展示，同时也提供了两个能力，一：获取省略号的起始位置；二：获取被省略文字的长度。
```java
/**
...
#### Layout异步执行
Layout是异步执行的，在TextView还没有计算出它的高度前调用layout的方法对其设置文本必然会发生奔溃现象。View的post()方法可以保证新任务是在layout调用过后执行，此处可以通过此方法避免该问题。
```java
