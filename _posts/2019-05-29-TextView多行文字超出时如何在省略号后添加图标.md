...
`方案1`实现效果其实跟在控件后面（周围）加图片标签的性质是一样的，**文字多行显示时**无法满足图标紧跟文字显示的需求，单行标题的模板可以使用此方案。

`方案2`是通过将HTML内容转化为Spanned格式在TextView中进行显示，它的输出是SpannableStringBuilder，实现原理跟`方案3`一样。它有特定的应用场景，所以此处不做过多描述，列出核心源码就能理解。
```java
...
#### Layout属性
TextView的Layout属性管理了文本的布局展示，同时也提供了两个能力，一：省略号开始的位置；二：被省略文字的长度。
```java
/**
...
int ellipsisStart = mTvTitle.getLayout().getEllipsisStart(mTvTitle.getLineCount() - 1);
if (ellipsisCount > 0) {
    text = text.substring(0, ellipsisStart - 1) + "…(精)";
}
SpannableString imageString = new SpannableString(text);
...
```
